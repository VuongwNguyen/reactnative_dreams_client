import { Pressable, Styleheet, Text, TouchableOpacity, View, TextInput, Image, } from 'react-native'
    ...
    <View style={createGroupChatStyle.container}>
      <View style={createGroupChatStyle.headerContainer}>
        <View style={createGroupChatStyle.headerLeft}>
          ...
          <Text style={createGroupChatStyle.titleHeader}>{t("createGroupScreen.newgroup")}</Text>
        </View>
        <Pressable style={createGroupChatStyle.buttonCreate}>
          <Text style={createGroupChatStyle.textButtonCreate}>{t("createGroupScreen.create")}</Text>
        </Pressable>
      </View>
      ...
      <View style={createGroupChatStyle.inputSearchContainer}>
        <View style={createGroupChatStyle.wraperInputSearch}>
          ...
          />
          <Pressable style={createGroupChatStyle.iconSearch} onPress={() => inputSearchRef.current.focus()}>
                ...
                <Pressable key={index}>
                  <View style={createGroupChatStyle.itemContainer}>
                    <View style={createGroupChatStyle.avatarContainer}>
                      <Image source={{ uri: user.avatar }} style={createGroupChatStyle.avatarItem} />
                      <Pressable onPress={() => handleDeleteUser(user)} style={createGroupChatStyle.buttonCross}>
      ...
      <View style={{ flex: 1, paddingHorizontal: 10 }}>
        <Text style={createGroupChatStyle.textSuggest}>{'\t'}{t("createGroupScreen.suggest")}</Text>
        <View style={{ height: 10 }} />
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={{ gap: 15 }}>
            {
              fakeDateUser.map((user, index) => (
                <View key={index} style={createGroupChatStyle.itemContainerSuggest}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                    <Image source={{ uri: user.avatar }} style={createGroupChatStyle.avatarItem} />
                    <Text style={createGroupChatStyle.textName}>{user.name}</Text>
                  </View>
                  <Pressable onPress={() => handleCheckboxPress(user)} style={[createGroupChatStyle.checkbox, { borderWidth: selectedUsers.includes(user.id) ? 0 : 1, backgroundColor: selectedUsers.includes(user.id) ? '#0cbbf0' : '#fff' }]}>
